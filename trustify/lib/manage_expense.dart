...
//   final dynamic title;
//
//   const ManageExpensePage({super.key, required this.title});
//
//   @override
...
//
//     Fluttertoast.showToast(msg: "Expencse Added Successfully");
//
...
//   bool _validate() {
//     setState(() {
//       _categoryError = categoryController.text.trim().isEmpty
...
//     }
//
//     final Uri urls = Uri.parse('$url/user_manage_expense/');
//
//     try {
//
...
//   }
//
//   // ── modernInputDecoration (unchanged, kept for compatibility)
...
//                       _buildLabel('Quick Select Category'),
...
class _ManageExpensePageState extends State<ManageExpensePage> {
  ...
  bool _validate() {
    ...
    return _categoryError == null && _amountError == null && _dateError == null;
  }
  ...
  void send_data() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('$url/user_manage_expense/'),
        body: {
          'lid': lid,
          'category': categoryController.text,
          'amount': amountController.text,
  ...
  Future<void> fetchExpenses() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('$url/user_view_expense/'),
        body: {'lid': lid},
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['status'] == 'ok') {
          setState(() {
            expenseList = List<Map<String, dynamic>>.from(jsonData['data']);
          });
        }
      }
    } catch (e) {
  ...
  Future<void> deleteExpense(String id) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('$url/user_delete_expense/'),
        body: {'expense_id': id},
      );
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: _categoryOptions.map((cat) {
                          final bool isSelected =
                              categoryController.text == cat['label'];
                          return GestureDetector(
                            onTap: () {
                              setState(() {
                                categoryController.text = cat['label'];
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          if (!isFetching && expenseList.isEmpty)
            _buildEmptyState()
          else
            ListView.builder(
              shrinkWrap: true,
  ...
  Widget _buildExpenseCard(Map<String, dynamic> item) {
    final IconData icon = _getCategoryIcon(item['category'] ?? '');
    return Container(
